...
function signatureForNode(node) {
  ...
  if (type === Node.ELEMENT_NODE) {
    ...
    // register as distinct while avoiding full subtree serialization. Include
    // the total text length so that streamed additions past the sampled prefix
    // still invalidate the signature; otherwise long replies would share the
    // same hash and appear truncated.
    const textSample = textContent.slice(0, TEXT_SAMPLE_LIMIT);
    const textHash = textSample ? hashString(textSample) : "";
    const textLength = textContent.length;

    return `el:${tag}:${node.childElementCount}:${attributeHash}:${textHash}:${textLength}`;
  }
  return `node:${type}`;
}
