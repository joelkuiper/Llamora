# TODO
- [x] What to do with tags
  - [x] Transitions for tags page / indicators
  - [x] Scroll jank 🙄
  - [x] Unify lazy scroll behavior between tags and diary
  - [x] Unify entries / response threads between diary and tags
- [x] Recovery code generation page rethink
- [x] Summaries for days as "tooltip" in calendar nav
- [x] Bugfix for multi token search
- [x] Performance issue tag summarization?
- [x] Security: get rid of cached summaries in localstorage/session storage, implement generic lockbox with api.
- [ ] README
Later
- [ ] Structure or other fancy viz view
- [ ] Better model than Qwen4B, find something that's a bit more chatty
  - [ ] https://huggingface.co/bartowski/Meta-Llama-3.1-8B-Instruct-GGUF seems to work better
  - [ ] "Prompt engineering"
- [x] Generate demo data / test data set
- [ ] Tests
- [ ] DB backups
- [ ] Help / introduction flow
- [ ] Better deploy/devops story
  - [ ] Versioning / CI / releases, etc
- [ ] Keyboard accessibility rethink
- [ ] User settings (font, theme, prompts?, llm settings?, etc)
- [ ] Image support?
- [ ] Generic attachment support?
Maybe
- [ ] Github / squarespace page, domain, etc
- [ ] Desktop app (Electron/Tauri)
