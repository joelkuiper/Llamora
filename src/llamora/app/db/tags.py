...
class TagsRepository(BaseRepository):
    ...
    async def get_tags_index_count_page(
        self,
        user_id: str,
        dek: bytes,
        *,
        ...
        cursor_hash_hex: str | None = None,
    ) -> tuple[list[dict[str, Any]], str | None, bool]:
        """Return a cursor-paginated tags index page sorted by usage count."""

        if limit <= 0:
            return [], None, False

        direction = "desc" if str(sort_dir).strip().lower() == "desc" else "asc"
        is_desc = direction == "desc"
        cursor_hash = (cursor_hash_hex or "").strip().upper() or None
        fetch_limit = limit + 1

        if is_desc:
            where_op = "<"
            order = "entry_count DESC, hash_hex DESC"
        else:
            where_op = ">"
            order = "entry_count ASC, hash_hex ASC"
        ...
        async with self.pool.connection() as conn:
            cursor = await conn.execute(
                ...
                SELECT tag_hash, hash_hex, name_ct, name_nonce, alg, entry_count
                FROM counts
                WHERE (
                  ? IS NULL
                  OR entry_count {where_op} ?
                  OR (entry_count = ? AND hash_hex {where_op} ?)
                )
                ORDER BY {order}
                LIMIT ?
                """,
            ...
            index_rows.append(
                {
                    ...
                    "hash": row["tag_hash"].hex(),
                    "count": int(row["entry_count"] or 0),
                    "hash_hex": str(row["hash_hex"] or "").lower(),
                }
            )
        ...
        if has_more and page_rows:
            last = page_rows[-1]
            next_cursor = (
                f"{int(last['entry_count'] or 0)}|{str(last['hash_hex'] or '').lower()}"
            )
        return index_rows, next_cursor, has_more
