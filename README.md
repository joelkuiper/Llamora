...
- **Limited Scalability:** Llamora mirrors the llama.cpp/llamafile server's available parallel slots (configure via `LLAMORA_LLM__SERVER__PARALLEL` or the server's `--parallel` flag). Without tuning this often means a single in-flight request. For heavy traffic, consider running multiple instances or a dedicated model service.
